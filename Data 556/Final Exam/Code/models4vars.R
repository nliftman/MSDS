...
M1Four = dmod(~ssc2+ssc4+time+ic,data=mytable)
M2Four = stepwise(M1Four,type="unrestricted",criterion="aic",search="all",direction="forward")

propFour = numeric(length(propSamples))
for(j in seq_len(length(propSamples)))
{
  mycount = 0
  for(i in seq_len(nTries))
  {
    atable = table(data[sample.int(nrow(data),size=propSamples[j]*nrow(data)),c("ssc2","ssc4","time","ic")])
    
    aM1forward = dmod(~ssc2+ssc4+time+ic,data=atable)
    ...
    if(isSameModel(aM2forward$glist,M2Four$glist)&&isSameModel(aM2backward$glist,M2Four$glist))
    {
      mycount = mycount+1
    }
  }
  propFour[j] = mycount/nTries
}

save(M1Four,M2Four,propFour,file="models4vars.Rdata")